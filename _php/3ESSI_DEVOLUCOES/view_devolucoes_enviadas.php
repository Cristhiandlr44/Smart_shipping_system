...
<head>
    ...
    <style>
        ...
        /* Ajuste específico para a coluna "Motivo" */
...
<body>    
                                    ...
                                    if ($result->num_rows > 0) {
                                        ...
                                        while ($row = $result->fetch_assoc()) {
                                            ...
                                            $anomaliasSql = "SELECT 
                                                                ...
                                                                a.devolvida,
                                                                a.observacao,
                                                                DATE_FORMAT(a.data_devolucao, '%d/%m/%Y') AS data_devolucao,
                                                                m.placa_caminhao,
                                                                m.finalizada
                                                            FROM 
                                                                anomalias a
                                            ...
                                            while ($anomalia = $anomaliasResult->fetch_assoc()) {
                                                if ($anomalia['tipo'] != 'reentrega' && $anomalia['devolvida'] ='S') { 
                                                        ...
                                                        <td>{$anomalia['data_devolucao']}</td>
                                                        <td>
                                                            <button class='btn btn-warning' onclick='alterarDevolvidaEstoque(\"{$anomalia['nf_anomalia']}\")'>Cancelar</button>
                                                        </td>
                                                    </tr>";
                                                }
                                            }
    ...
    <script>
        ...
        function alterarDevolvidaEstoque(nf_anomalia) {
            // Alterar o texto na tabela para 'S' (antes de enviar a requisição, para melhorar a experiência do usuário)
            document.getElementById('devolvida-' + nf_anomalia).innerText = 'N';

            // Enviar uma requisição AJAX para atualizar o banco de dados
            $.ajax({
                url: 'alterarDevolvidaEstoque.php', // Arquivo PHP que irá atualizar o banco
                type: 'POST',
                data: {
                    nf_anomalia: nf_anomalia, // Passa a nota fiscal da anomalia
                    devolvida: 'N' // Valor a ser alterado
                },
                success: function(response) {
                    // Atualizar a tabela, se necessário
                    // (Neste caso, já alteramos o texto para 'S' antes da requisição, então podemos apenas mostrar o sucesso)
                    alert("Devolvida alterada com sucesso!");
                    console.log(response);
                },
                error: function() {
                    // Reverter a mudança na tabela se houver erro
