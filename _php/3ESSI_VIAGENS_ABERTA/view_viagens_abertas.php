...
<head>
    ...
    .btn-finalizar:hover {
        ...
        /* Efeito de aumento de tamanho */
    }

    .btn-finalizar:active {
        background-color: #1e7e34 !important;
        /* Tom de verde ainda mais escuro */
    }
    ...
    .table td .btn-group button {
        flex: 0 0 auto;
        /* Garante que os botões não se estiquem */
        max-width: 150px;
        /* Limita a largura do botão para evitar quebra excessiva */
    }
    ...
    .table td:last-child {
        width: 400px;
        /* Ajuste para a largura da coluna de ações */
    }
    ...
    .btn-acao {
        ...
        margin-bottom: 10px;
        width: 100%;
        /* Faz o botão "Ação" ocupar toda a largura disponível */
    }
    ...
    <?php require("../elements/cssNavbar.php");
    ?>
    </style>
...
<body>
                    ...
                    <table class="table table-striped table-bordered text-nowrap" id="viagensAbertasTable">
                        ...
                        <tbody>
                            <?php
                                ...
                                $sql = "SELECT m.id, mo.nome AS motorista, mc.fk_placa AS placa_caminhao, m.status, m.largada, m.observacoes, MAX(n.rota) AS rota
                                        ...
                                        JOIN notas  AS n ON n.id_monitoramento = m.id

                                        WHERE m.finalizada IS NULL
                                        GROUP BY n.rota" ;
                                ...
                                if ($result->num_rows > 0) {
                                    while ($row = $result->fetch_assoc()) {
                                        ...
                                        $notasSql = "SELECT n.n_nota, c.nome AS cliente, n.bairro, n.cidade, n.peso_bruto 
                                                    ...
                                                    WHERE n.id_monitoramento = {$row['id']} OR id_monitoramento2 = {$row['id']} OR id_monitoramento3 = {$row['id']}" ;
                                        ...
                                        echo "<tr>
                                                    ...
                                                    <td>
                                                        ...
                                                        <div class='acoes-{$row['id']}' style='display: none;'>
                                                            <button class='btn btn-anomalia' onclick='apontarAnomalia({$row['id']})'>Apontar Anomalia</button>
                                                            <button class='btn btn-anomalia' onclick='listarAnomalias({$row['id']})'>Listar Anomalia</button>
                                                            <button class='btn btn-finalizar' onclick='finalizarViagem({$row['id']})'>Finalizar Viagem</button>
                                                            <button class='btn btn-alterar-placa' onclick='alterarPlaca({$row['id']})'>Alterar Placa</button>
                                                        </div>
                                                    </td>
                                                </tr>";
                                        ...
                                        echo "<tr id='notas-{$row['id']}' class='notas-list '>
                                                <td colspan='8'>
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"
                        onclick="fecharTela()"></button>
                </div>
                <div class="modal-body">
                    <div class="form-group">
                        <label for="anomaliaTipo">Tipo de Anomalia</label>
                        <select class="form-control" id="anomaliaTipo"
                            onchange="carregarNotasFiscais(viagemIdGlobal); mostrarOpcaoAnomalia();">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal"
                        onclick="fecharTela()">Fechar </button>
                    <button type="button" class="btn btn-primary" id="saveAnomalia"
                        onclick="salvarAnomalia()">Salvar</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <!-- Modal Alterar Placa -->
    <div class="modal fade" id="alterarPlacaModal" tabindex="-1" aria-labelledby="alterarPlacaModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
    ...
    <div class="modal fade" id="itensModal" tabindex="-1" role="dialog" aria-labelledby="itensModalLabel"
        aria-hidden="true">
    ...
    function carregarItensNota() {
        ...
        if (notaFiscal) {
            $.ajax({
                ...
                }, // Passando o número da nota fiscal
                success: function(response) {
            ...
            content = `
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="motivoAnomalia" required>
                            ...
                            <option value="14-Mercadoria Invertida">Mercadoria Invertida</option>
                            <option value="15-Item sem cadastro">Item sem cadastro</option>
                            <option value="16-Cliente Fora de Rota">Cliente fora de rota</option>
    ...
    function salvarAnomalia() {
        var nf = document.getElementById('notaFiscal') ? document.getElementById('notaFiscal').value :
            null; // Verifica se o elemento existe
        var tipo = document.getElementById('anomaliaTipo').value;
        var motivo = document.getElementById('motivoAnomalia').value;
        var quantidade = parseFloat(document.getElementById('quantidadeAnomalia') ? document.getElementById(
            'quantidadeAnomalia').value : 0);
        var unidade = document.getElementById('unidadeAnomalia') ? document.getElementById('unidadeAnomalia').value :
            null;
        var peso = parseFloat(document.getElementById('pesoAnomalia') ? document.getElementById('pesoAnomalia').value :
            0);
        ...
        if (tipo === 'devolucaoParcial') {
                    ...
                    try {
                        ...
                        if (data.success) {
                            ...
                            if (quantidade > maxQuantidade) {
                                alert(
                                    `A quantidade informada (${quantidade}) não pode ser maior do que a disponível (${maxQuantidade}).`
                                );
                                return;
                            }
                            if (peso > maxPeso) {
                                alert(
                                    `O peso informado (${peso}) não pode ser maior do que o disponível (${maxPeso}).`
                                );
                                return;
                            }
            ...
            if (confirm("Você deseja realmente remover o monitoramento dessa nota fiscal?")) {
                // Fazer uma chamada AJAX para processar a reentrega
                $.ajax({
                    url: 'removerMonitoramento.php', // Arquivo PHP que processa a reentrega
                    type: 'POST',
                    data: {
                        notaFiscal: nf
                    },
                    success: function(response) {
                        try {
                            ...
                            if (data.success === true) {
                                ...
                                alert(data.message || "Erro ao remover o monitoramento.");
                            }
                        } catch (e) {
                            console.error("Erro ao processar a resposta do servidor:", e);
                            alert("Erro ao processar a resposta do servidor.");
                ...
                try {
                    ...
                    if (data.success) {
                        ...
                        notasFiscais.forEach(function(nf) {
                            console.log("salve");
                            listarAnomaliasPorNotaFiscal(
                                nf); // Lista as anomalias para cada nota fiscal
                        });
                ...
                try {
                    ...
                    if (data.success) {
                        ...
                        anomalias.forEach(anomalia => {
                            if (anomalia.tipo === "devolucaoTotal" || anomalia.tipo ===
                                "reentrega") {
                                ...
                                li.className =
                                    'list-group-item d-flex flex-column align-items-start mb-3';
                                li.innerHTML = `
                                        <div class="card p-3">
                ...
                try {
                    ...
                    if (data.success) {
                        ...
                        const liToRemove = document.querySelector(
                            `button[onclick="excluirAnomalia(${id})"]`).closest('li');
                        if (liToRemove) {
                            liToRemove.remove();
                        }
                    } else {
            ...
            success: function(response) {
                ...
                try {
                    ...
                    if (data.success) {
                        ...
                        // Remover a anomalia da lista na interface
                        const liToRemove = document.querySelector(
                            `button[onclick="excluirAnomaliaTotal(${id}, '${nf}')"]`).closest('li');
                        if (liToRemove) {
                            liToRemove.remove();
                        }
                    } else {
