...
<body>
    ...
    <div class="container">
        <div class="dash bg-white">
            <div class="row p-3">
                <?php
                    $title = "- Base Rota Bairros";
                    require_once($relative . "elements/tituloProjetoMainSection.php");

                    if ($_SESSION['tipo'] == 1) {
                        echo "
                        <div class='row'>
                            <p class='col-sm-8 offset-sm-2 col-md-11 offset-md-1' style='font-size: 1.5em;text-align: center;'>Atualize o Base Rotas Bairros</p>
                        </div>";
                    }
                ?>
            </div>
                ...
                <div class="col-md-12">
                    <form id="formAdicionarRota">
                        <div class="row">
                            ...
                            <div class="col-md-5">
                                <input type="text" id="novoBairro" class="form-control" placeholder="Digite o bairro" required>
                            </div>
                            <div class="col-md-5">
            ...
            <div class="row">
                <div id="table" class="table table-striped table-bordered" style="width: 100%;">
                    <table id="viagensAbertasTable" class="table table-striped table-bordered">
                        ...
                        <tbody>
                            <?php
                            $sql = "SELECT * FROM base_rotas_bairros";
    ...
    <script>
        $(document).ready(function() {
            ...
            // Submissão do formulário para adicionar cidade bairro e rota
            $('#formAdicionarRota').on('submit', function(e) {
                e.preventDefault(); // Impede o envio padrão do formulário
                const cidade = $('#novaCidade').val();
                const bairro = $('#novoBairro').val();
                const rota = $('#novaRota').val();

                $.ajax({
                    url: 'adicionar_rota_bairro.php', // Arquivo PHP para inserir os dados
                    type: 'POST',
                    data: { cidade, bairro,rota },
