...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $sql = "SELECT 
            ...
            ORDER BY n.rota, fornecedor, p.cod ASC, nf ASC, n.reentrega DESC";
    ...
    if ($stmt && $stmt->execute()) {
        ...
        if (!empty($produtos)) {
            ...
            // Gerar uma página para cada rota
            foreach ($produtosPorRota as $rota => $lista) {
                $pdf->AddPage();

                // Calculando peso e volume total por rota
                $totalPeso = array_sum(array_column($lista, 'Peso'));
                $totalVolume = array_sum(array_column($lista, 'quantidade'));
